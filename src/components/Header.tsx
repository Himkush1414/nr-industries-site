...
/**
 * Site-wide navigation bar — promoted from /lab/lv9's Lv9Nav.tsx (see
 ...
 * tabs/actions groups are replaced by a hamburger button opening a full-
 * screen menu.
 *
 * `NAV_HEIGHT` is exported so Layout.tsx can reserve that much top space
 * for every route except Home (whose hero is designed to sit flush behind
 * this bar, the same way LV3's hero does) — being `position: fixed` (not
 * `sticky`, unlike this file's predecessor), it reserves no space in
 * normal document flow on its own.
 */
...
export function Header() {
  ...
  return (
    ...
    {/* Full-screen mobile menu — a SIBLING of <nav>, not a descendant:
        <nav> has `backdrop-filter`, which (like `transform`) creates a new
        ...
        to that 90px box instead of the viewport, collapsing to zero
        height. Living outside <nav> avoids that entirely, positioning
        normally relative to the viewport. Opaque (not blurred-transparent
        like the bar itself) so its own content stays readable over
    ...
    {menuOpen && (
        <div
          style={{
            ...
            padding: "32px clamp(16px, 6vw, 30px)",
            gap: "28px",
            overflowY: "auto",
          }}
        >
          <span
            ...
            style={{ fontSize: "clamp(16px, 4vw, 18px)", fontWeight: 700, color: TEXT_CREAM }}
          >
            NR Industries
          </span>
            ...
            {TABS.map((tab) => (
              <NavLink
                ...
                style={({ isActive }) => ({
                  ...
                  fontWeight: 600,
                  color: isActive ? GOLD_ACCENT : TEXT_CREAM,
                  textDecoration: "none",
                })}
              >
                {tab.label}
