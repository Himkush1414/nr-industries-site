...
export function HomePage() {
  ...
  // Change #4 (hero background parallax): the background image scrolls at a
  // fraction of the page's scroll speed, so it appears to lag behind/stay
  // put relative to the foreground content as the user scrolls past the hero.
  // Isolated to the hero's own <img> ref — doesn't touch the headline fade
  // or the navbar's own independent scroll handling below.
  useEffect(() => {
  ...
  // Change #5 (hero heading scroll fade): the "NR Industries" headline fades
  // out as the page scrolls, independent of the navbar's own logo fade
  // (separate ref, separate effect, separate scroll handler).
  useEffect(() => {
  ...
  return (
    <>
      {/* Hero */}
      <section className="relative -mt-20 flex min-h-screen flex-col justify-center overflow-hidden bg-navy-950">
        <img
          ref={heroImageRef}
      ...
      {/* Change #6: stats section moved here, directly below the "Built to a
          standard, not just a spec." about teaser above — was previously
