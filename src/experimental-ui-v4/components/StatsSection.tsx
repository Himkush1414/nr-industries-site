 ...
 * Specifications page) — the one dataset that's genuinely comparable as
 * proportional bars (same "units/annum" unit throughout). */
  ...
  useLayoutEffect(() => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnter: () => {
        bars.forEach((bar, i) => {
          const pct = bar.dataset.pct;
          gsap.to(bar, { scaleX: pct ? Number(pct) / 100 : 1, duration: 1.1, delay: i * 0.08, ease: "power3.out" });
        });
      },
    });
    return () => trigger.kill();
    ...
    <section className="py-24 sm:py-32">
      <div className="container-page flex flex-col gap-12">
        <GsapReveal className="mx-auto flex max-w-2xl flex-col items-center gap-4 text-center">
          <span className="v4-micro-label">Production Capacity</span>
            ...
            return (
              <div key={row.rating} className="flex flex-col gap-2">
                <div className="flex items-baseline justify-between text-sm">
                  <span className="v4-fg font-semibold">{row.rating}</span>
                  <span className="v4-fg-dim">{row.unitsPerAnnum}</span>
                </div>
                <div className="v4-bar-track">
                  <div className="v4-bar-fill" data-bar data-pct={pct} />
                </div>
              </div>
            );
          })}
